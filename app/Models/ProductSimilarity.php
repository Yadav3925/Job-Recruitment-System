...
class ProductSimilarity
{
    ...
    public function __construct(array $products)
    {
        ...
        $this->priceHighRange = max(array_column($products, 'salaryrange'));
    }
    ...
    {
       // $a=(array) $productA;
          $productAFeatures = "IT and techonolgy";
          $productBFeatures =  "Human Resourse";
          $productCFeatures = "Education";
          $productDFeatures =  "Customer Services";
      //dd($productB);
        return array_sum([
            (Similarity::hamming($productAFeatures, $productBFeatures) * $this->featureWeight),
            (Similarity::euclidean(
                Similarity::minMaxNorm([$productA['salaryrange']], 0, $this->priceHighRange),
                Similarity::minMaxNorm([$productB['salaryrange']], 0, $this->priceHighRange)
            ) * $this->priceWeight),
            (Similarity::jaccard($productA['post'], $productB['post']) * $this->categoryWeight)
